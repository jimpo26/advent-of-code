...
for line in lines:
    if "$ cd" in line:
        ...
        if ".." in line:
            ...
            # i added some random byte to the end of the key to avoid problems if there are folders with the same name
...
available_size = TOTAL_SIZE - get_size(tree)
ok_folders = []
for x in sizes:
    if sizes[x] > (SIZE_REQUIRED - available_size):
        ok_folders.append(sizes[x])
print(min(ok_folders))
